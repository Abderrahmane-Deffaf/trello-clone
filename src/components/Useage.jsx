    ...
    <div>
      <div className="max-w-[1320px] m-auto p-4 flex flex-col gap-5">
        <div className="flex flex-col max-w-[60%] gap-4">
          <p className="font-medium">DES FONCTIONNALITÉS POUR ÉVOLUER EFFICACEMENT</p>
            ...
            infos.map((Element) => {
              return (
                <div className="griddisplay bg-lightgray p-4 gap-4 rounded-xl basis-[32%]">
                  ...
                  <p>{Element.text}</p>
                  <a className="border-blue hover:bg-lightblue border w-fit p-6 bg-white  rounded-xl flex  items-center">{Element.link}</a>
                </div>
              );
            })
          }
